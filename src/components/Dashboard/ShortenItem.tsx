...
const ShortenItem = ({ originalLink, shortLink, clickCount, createdDate }: { [key: string]: any }) => {
    ...
    const analyticsHandler = (shortLink: any) => {
        if (!analyticToggle) {
            setSelectedUrl(shortLink);
        }
        setAnalyticToggle(!analyticToggle);
    }

    const fetchMyshortLink = async () => {
        setLoader(true);
        try {
            ...
            <div className={`flex sm:flex-row flex-col  sm:justify-between w-full sm:gap-0 gap-5 py-5 `}>
                <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
                    <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2 ">
                        {/* <a href={`${import.meta.env.VITE_REACT_SUBDOMAIN}/${shortLink}`}
                target="_blank"
                ...
                {subDomain + "/" + `${shortLink}`}
                        ...
                        <Link
                            target='_'
                            className='text-[17px]  font-montserrat font-[600] text-linkColor'
                            to={import.meta.env.VITE_REACT_FRONT_END_URL + "/s/" + `${shortLink}`}>
                            {subDomain + "/s/" + `${shortLink}`}
                        </Link>
                ...
                <div className="flex  flex-1  sm:justify-end items-center gap-4">
                    <CopyToClipboard
                        ...
                        text={`${import.meta.env.VITE_REACT_FRONT_END_URL + "/s/" + `${shortLink}`}`}
                    >
                    ...
                    <div
                        onClick={() => analyticsHandler(shortLink)}
